<template>
    <div class=" w-full divide-y">
        <div class=" w-full flex flex-col items-start space-y-4 pb-8" :class="{'pt-8': index > 0}" v-for="(item,index,key) in form" :key="key">
            <span class=" text-[18.75px] font-medium font-montserrat text-secondary">Organisasi #{{index + 1}}</span>
            ...
            <div class=" grid grid-cols-2 gap-x-4">
                <Input  label="Jabatan" placeholder="Ex : Manajer" v-model="item.Jabatan"/>
                <Input  label="Lokasi" placeholder="Ex : Medan" v-model="item.Lokasi"/>
            </div>
         ...
         <button @click="AddOrganisasi" class=" bg-background p-3 rounded-xl hover:bg-opacity-75">
                <PluseIcon />
         </button>
